...
public:
    ...
    int data = 10;//C++11标准后才支持
};

int test1(void)
{
    auto f1 = bind(func, 1, 2);
    cout << "1 + 2 = " << f1() <<  endl;

	//还可以绑定到成员函数上
    auto f2 = bind(&A::func, A(), 1, 2);
    cout << "f2() = " << f2() << endl;

    A a;
    //对于不提前绑定的参数，要使用占位符_1, _2, _3, _4, ... 占据
    auto f3 = bind(&A::func, &a, 1, std::placeholders::_1);
    f3(10);
    return 0;
}
...
int test2(void)
{
    ...
    int n = 7;
    //1. 使用bind时，占位符占据的时形参的位置
    //2. 占位符本身的数字代表的是实参传递的位置，
    // 可以把占位符理解成是变量，该变量的值是由
    //实参传递进来的，占位符本身的数字时几，就
    //绑定到相应的实参传递时的位置
